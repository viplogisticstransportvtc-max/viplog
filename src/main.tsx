...
const requirements = [
  ...
  "Must be able to attend 5 convoys OR complete 1 convoy + log 2000 KM each month.",
      ...
      <header className="fixed top-0 z-50 w-full border-b border-white/10 bg-[#080808]/75 backdrop-blur-xl">
        <div className="mx-auto flex max-w-7xl items-center justify-between px-5 py-3 lg:px-8">
          ...
          <nav className="hidden items-center gap-5 xl:flex">
            {["Home","About Us","Requirements","Fleet","Drivers","Convoys","Gallery","News","Contact"].map((item) => (
...
function AdminApp() {
  ...
  const [tab, setTab] = useState<"dashboard"|"drivers"|"fleet"|"convoys"|"news"|"gallery">("dashboard");
  const [loggedIn, setLoggedIn] = useState(false);
  const [username, setUsername] = useState("");
  const [password, setPassword] = useState("");
  const [message, setMessage] = useState("");

  useEffect(() => { fetch('/api/admin?action=session').then(r=>r.json()).then(data=>setLoggedIn(!!data.authenticated)).catch(()=>setLoggedIn(false)); }, []);
