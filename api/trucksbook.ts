...
export async function POST(request:Request){
 try{
  ...
  if(records.length){
   const ins=await supabase("delivery_records?on_conflict=external_id",{
    method:"POST",
    body:JSON.stringify(records),
    headers:{Prefer:"resolution=ignore-duplicates,return=representation"}
   });
   if(!ins.ok){console.error(await ins.text());return json({error:"Unable to import delivery records into Supabase."},500);}
   imported=(await ins.json()).length;
  }
